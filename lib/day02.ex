defmodule Advent.Day02 do
  ...
  def checksum(input, fun) do
    input
    |> String.split("\n")
    |> Enum.map(&String.split/1)
    |> Enum.map(fn x -> x |> Enum.map(&String.to_integer/1) |> Enum.sort() end)
    |> Enum.map(fun)
    |> Enum.sum()
  end

  def difference_largest_smallest([]), do: 0

  def difference_largest_smallest([head | tail]) do
    largest = List.last(tail)
    smallest = head
    largest - smallest
  end

  def difference_divisor([]), do: 0

  def difference_divisor([head | tail]) do
    case Enum.find(tail, fn x -> rem(x, head) == 0 end) do
      nil -> difference_divisor(tail)
      x -> div(x, head)
    end
  end

  def part_1(input), do: checksum(input, &difference_largest_smallest/1)
  def part_2(input), do: checksum(input, &difference_divisor/1)
